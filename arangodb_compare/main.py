...
def get_db_entity_counts(db1: StandardDatabase, db2: StandardDatabase, log_dir: str) -> None:
    ...
    for entity in entity_types:
        console.print(f"[bold cyan]Checking {entity}...[/bold cyan]")
        ...
        table = Table(title=f"{entity.capitalize()} Counts", show_header=True, header_style="bold magenta")
        table.add_column("DB1", style="dim", justify="center")
        table.add_column("DB2", style="dim", justify="center")
        table.add_row(str(count_db1), str(count_db2))
        console.print(table)
        ...
        if count_db1 != count_db2:
            ...
            console.print(f"[red]{mismatch_message}[/red]")
...
def compare_collection_counts(db1: StandardDatabase, db2: StandardDatabase, log_dir: str) -> None:
    collection_names_db1 = get_entity_names(db1, 'collections')
    console.print(f"[bold cyan]Checking collection document counts...[/bold cyan]")
    write_log(log_dir, "collections", "Collection Document Counts", "h2")
    for collection_name in collection_names_db1:
        if collection_name not in get_entity_names(db2, 'collections'):
            console.print(f"[yellow]Skipping {collection_name} (not found in DB2)[/yellow]")
            write_log(log_dir, "collections", f"Skipping {collection_name} (not found in DB2)", "h4")
            continue

        try:
            count1 = get_collection_count(db1, collection_name)
        except Exception as e:
            console.print(f"[red]Error getting count for {collection_name} in db1: {e}[/red]")
            continue

        try:
            count2 = get_collection_count(db2, collection_name)
        except Exception as e:
            console.print(f"[red]Error getting count for {collection_name} in db2: {e}[/red]")
            continue

        if count1 != count2:
            mismatch_message = f"Mismatch in collection '{collection_name}': {count1} vs {count2}"
            console.print(f"[bold red]{mismatch_message}[/bold red]")
...
def compare_collection_indexes(db1: StandardDatabase, db2: StandardDatabase, log_dir: str) -> None:
    collection_names_db1 = get_entity_names(db1, 'collections')
    collection_names_db2 = get_entity_names(db2, 'collections')

    console.print(f"[bold cyan]Checking indexes for collections...[/bold cyan]")
    write_log(log_dir, "indexes", "Collection Indexes", "h2")

    for collection_name in collection_names_db1:
        if collection_name not in collection_names_db2:
            console.print(f"[yellow]Collection {collection_name} does not exist in db2. Skipping.[/yellow]")
            write_log(log_dir, "indexes", f"Collection {collection_name} not found in db2", "h4")
            continue
